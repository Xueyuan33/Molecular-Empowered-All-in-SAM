...
def get_arguments():
    ...
    parser.add_argument("--valset_dir", type=str, default='/Data2/KI_data_testingset_patch/data_list_testing.csv')
    # parser.add_argument("--valset_dir", type=str, default='/Data2/KI_data_validationset_patch/')
    parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/fold1_with_white_scale_segany_posposneg_5_0406_test/')
    parser.add_argument("--reload_path", type=str, default='snapshots_2D/fold1_with_white_scale_segany_posposneg_5_0406_test/MOTS_DynConv_fold1_with_white_scale_segany_0406_e100.pth')
...
def randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=3, comp_with_seed_rate=1.0): #        kernel = np.ones((3, 3), np.uint8)
   ...
   if 0 in large_components_idx: large_components_idx = np.delete(large_components_idx,
                                                                  np.where(large_components_idx == 0))
   ...
   background_component_mask = np.uint8(labels == 0) * 255
   x, y = np.where(background_component_mask == 255)
   for ii in range(10):
       i = np.random.randint(len(x))
       random_neg = [y[i], x[i]]
       output_neg.append(random_neg)
   ...
   for idx in large_components_idx:
       ...
       if mode == 'center':
           ...
           i = np.random.randint(len(x))
           random_pos = [y[i],x[i]]
           output_pos.append(random_pos)

   len_num = np.min([5, len(output_pos)])
   return output_pos[:len_num], output_neg[:len_num], mask_img
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        sam_checkpoint = "/Data2/HumanKidney/SegmentAnything/checkpoint/sam_vit_h_4b8939.pth"
        device = "cuda:0"
        model_type = "default"
        ...
        with torch.no_grad():
            for iter, batch in enumerate(valloader):
                ...
                output_folder = os.path.join(args.snapshot_dir.replace('snapshots_2D/fold1_with_white','/Data2/HumanKidney/SegmentAnything/testing_%s' % (args.validsetname)), str(args.best_epoch))
                ...
                if task0_pool_image.num_imgs >= batch_size:
                    ...
                    for ni in range(images.shape[0]):
                        ...
                        out_pos, out_neg, _ = randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=10,comp_with_seed_rate=1.0)
                        #out = randomseed_perComp((labels[ni].cpu().numpy() * 255).astype(np.uint8), min_comp_area=10, mode='random')

                        if len(out_pos) == 0:
                            out_pos, out_neg, _ = randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=0,comp_with_seed_rate=1.0)

                        input_point = np.concatenate((out_pos, out_neg), 0)
                        # input_point = np.array((out_pos, out_neg))
                        input_label = np.concatenate((np.ones((len(out_pos))), np.zeros((len(out_neg)))),0)
