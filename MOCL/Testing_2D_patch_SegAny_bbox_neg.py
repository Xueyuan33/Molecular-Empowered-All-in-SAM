...
def get_arguments():
    ...
    parser.add_argument("--valset_dir", type=str, default='/Data2/KI_data_testingset_patch/data_list.csv')
    # parser.add_argument("--valset_dir", type=str, default='/Data2/KI_data_validationset_patch/')
    parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/fold1_with_white_scale_segany_posposneg_0406/')
    parser.add_argument("--reload_path", type=str, default='snapshots_2D/fold1_with_white_scale_segany_posposneg_0406/MOTS_DynConv_fold1_with_white_scale_segany_0406_e100.pth')
...
def randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=3, comp_with_seed_rate=1.0): #        kernel = np.ones((3, 3), np.uint8)
   ...
   len_num = np.min([10, len(output_pos)])
            ...
            for iter, batch in enumerate(valloader):
                ...
                if task0_pool_image.num_imgs >= batch_size:
                    ...
                    for pi in range(len(images)):
                        ...
                        plt.axis('off')
                        plt.savefig(os.path.join(output_folder, filename[pi] + '_segany_%s.png' %(now_task.item())))
                        plt.close()
