...
def get_arguments():
    ...
    parser.add_argument("--valset_dir", type=str, default='/content/drive/MyDrive/After_correct/auto_label_test_set/data_list.csv')
    # parser.add_argument("--valset_dir", type=str, default='/Data2/KI_data_validationset_patch/')
    parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/fold1_with_white_scale_segany_bbox_0406_test/')
    parser.add_argument("--reload_path", type=str, default='snapshots_2D/fold1_with_white_scale_segany_bbox_0406_test/MOTS_DynConv_fold1_with_white_scale_segany_0406_e100.pth')
...
def randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=0, dilate_size = 3, comp_with_seed_rate=1.0): #        kernel = np.ones((3, 3), np.uint8)
   """
   ...
   for idx in large_components_idx:
       ...
       if mode == 'center':
           ...
           '''dilation the instance masks and get box'''
           # kernel = np.ones((dilate_size, dilate_size), np.uint8)
           # component_mask_dil = cv2.dilate(component_mask, kernel, iterations=1)
           #
           # cont, _ = cv2.findContours(component_mask.copy(), 1, 1)
           # rect = cv2.minAreaRect(cont[0])
           # (x, y), (w, h), a = rect
           # left top width heigh
           #left top width height
           #random_pos = [int(stats[idx,0]), int(stats[idx,1]), int(stats[idx,0] + stats[idx,2]), int(stats[idx,0] + stats[idx,3])]
           random_pos = [int(stats[idx,0]), int(stats[idx,1]), int(stats[idx,0] + stats[idx,2]), int(stats[idx,1] + stats[idx,3])]
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        with torch.no_grad():
            for iter, batch in enumerate(valloader):
                ...
                #output_folder = os.path.join(args.snapshot_dir.replace('/Data2/Omni-Seg_data/After_correct/auto_label_test_set','/content/drive/MyDrive/After_cottect/auto_label_test_set_%s' % (args.validsetname)), str(args.best_epoch))
                output_folder = os.path.join('/content/drive/MyDrive/ToXueyuan/', 'output_folder')
                ...
                if task0_pool_image.num_imgs >= batch_size:
                    ...
                    'SAM'
                    now_preds = torch.zeros((images.shape[0],images.shape[2],images.shape[3])).cuda()
                    ...
                    for ni in range(images.shape[0]):
                        ...
                        out,_ = randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=0, dilate_size = 10, comp_with_seed_rate=1.0)
                        #out = randomseed_perComp((labels[ni].cpu().numpy() * 255).astype(np.uint8), min_comp_area=10, mode='random')

                        if len(out) == 0:
                            out, _ = randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=0, dilate_size = 5, comp_with_seed_rate=1.0)

                        input_box = torch.tensor((out)).cuda()

                        if len(input_box) > 0:

                            transformed_boxes = model.transform.apply_boxes_torch(input_box, (now_img.permute([1,2,0]).cpu().numpy() * 255).astype(np.uint8).shape[:2])
                            masks_all, scores, logits = model.predict_torch(
                                    point_coords=None,
                                    point_labels=None,
                            ...
                            masks_all, _, _ = model.predict(
                                point_coords=None,
                                point_labels=None,
                                multimask_output=False,
                            )
                            masks = torch.from_numpy(masks_all)

                        input_box_list.append((input_box))

                        now_preds[ni] = masks[0].cuda()
                    ...
                    for pi in range(len(images)):
                        ...
                        if not (input_box_list[pi] is None):
                            #print(input_box_list[pi])
                            for bbx in range(len(input_box_list[pi])):
                                show_box(input_box_list[pi][bbx].cpu().numpy(), plt.gca())
                ...
                if task1_pool_image.num_imgs >= batch_size:
                    ...
                    for ni in range(images.shape[0]):
                        ...
                        out, _ = randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=0,
                                                    dilate_size=10, comp_with_seed_rate=1.0)
                        # out = randomseed_perComp((labels[ni].cpu().numpy() * 255).astype(np.uint8), min_comp_area=10, mode='random')

                        if len(out) == 0:
                            out, _ = randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=0,
                                                        dilate_size=5, comp_with_seed_rate=1.0)

                        input_box = torch.tensor((out)).cuda()

                        if len(input_box) > 0:

                            transformed_boxes = model.transform.apply_boxes_torch(input_box, (now_img.permute(
                                [1, 2, 0]).cpu().numpy() * 255).astype(np.uint8).shape[:2])
                            masks_all, scores, logits = model.predict_torch(
                                point_coords=None,
                                point_labels=None,
                                boxes=transformed_boxes,
