...
from MOTSDataset_2D_Patch_supervise_csv_ori import MOTSValDataSet as MOTSValDataSet_joint
...
def get_arguments():
    ...
    parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/fold1_with_white_scale_segany_point_0406_test/')
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        with torch.no_grad():
            for iter, batch in enumerate(valloader):
                ...
                output_folder = os.path.join(args.snapshot_dir.replace('snapshots_2D/fold1_with_white','/content/drive/MyDrive/After_cottect/auto_label_test_set_%s' % (args.validsetname)), str(args.best_epoch))
                ...
                if task0_pool_image.num_imgs >= batch_size:
                    ...
                    for ni in range(images.shape[0]):
                        ...
                        out,_ = randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=10,comp_with_seed_rate=1.0)
                        #out = randomseed_perComp((labels[ni].cpu().numpy() * 255).astype(np.uint8), min_comp_area=10, mode='random')

                        if len(out) == 0:
                            out, _ = randomseed_perComp(mask_img, min_comp_area=1, mode='random', erode_size=0,comp_with_seed_rate=1.0)
