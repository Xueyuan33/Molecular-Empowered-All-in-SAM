...
def calc_f1(y_pred,y_true):
    ...
    return f1/batchsize, auc/batchsize


def calc_binf1(y_pred,y_true):
    batchsize = y_true.shape[0]
    with torch.no_grad():
        assert y_pred.shape == y_true.shape
        f1, precision,recall,aji_score = 0, 0, 0, 0
        y_true = y_true.cpu().numpy()
        y_pred = y_pred.cpu().numpy()
        for i in range(batchsize):
            true = y_true[i].flatten()
            ...
            aji_score += jaccard_score(true, pred)


    return f1/batchsize, precision/batchsize, recall/batchsize, aji_score/batchsize
