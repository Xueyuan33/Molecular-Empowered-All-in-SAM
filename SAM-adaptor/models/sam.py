...
class masked_BBCEWithLogitLoss(nn.Module):
    '''
    Balanced BCEWithLogitLoss
    '''
    def __init__(self):
...
def _dice_loss(pred, target):
    pred = torch.sigmoid(pred) #self.pred_mask
    inter = (pred * target).sum(dim=(2, 3))
    union = pred.sum(dim=(2, 3)) + target.sum(dim=(2, 3))
    dice = 1 - (2 * inter / union.clamp(min=1e-8))
...
def _masked_dice_loss(pred, target, mask):
    ...
    inter = (masked_pred * masked_target).sum(dim=(2, 3))
    union = masked_pred.sum(dim=(2, 3)) + masked_target.sum(dim=(2, 3))
    dice = 1 - (2 * inter / union.clamp(min=1e-8))

    return dice.mean()


def _multidice_loss(pred,target):
    if True:
        pred = torch.sigmoid(pred)
        dice_total = 0.0
        for i in range(num_classes):
            inter = (pred[:, i, ...] * target[:, i, ...]).sum(dim=(1, 2))
...
def _dice_val(pred, target):
    #pred = torch.sigmoid(pred) #self.pred_mask
    inter = (pred * target).sum(dim=(2, 3))
    union = pred.sum(dim=(2, 3)) + target.sum(dim=(2, 3))
    dice = (2 * inter / union.clamp(min=1e-8))
...
class SAM(nn.Module):
    ...
    def loss_G_cathyAdd(self, pred, gt):
        """Calculate GAN and L1 loss for the generator"""
        self.loss_G = self.criterionBCE(pred, gt)
        if self.loss_mode == 'iou':
            self.loss_G += _iou_loss(pred, gt)
        if self.loss_mode == 'dice':
            self.loss_G += _dice_loss(pred, gt)
        return self.loss_G
